...
const FontSizeButton = () => {
  const { editor } = useEditorStore();
  // textStyle is a mark that can store font-size
...
return (
  <div className="flex items-center gap-x-0.5">
    <button
      className="h-7 w-7 shrink-0 flex items-center justify-center rounded-sm hover:bg-neutral-200/80 dark:hover:bg-neutral-700/80"
      onClick={decrement}
    >
    ...
    {isEditing ? (
      <input
        ...
        className="
          h-7 w-10 text-sm text-center rounded-sm border border-neutral-300 
          bg-white dark:bg-neutral-800 dark:text-white dark:border-neutral-600
...
const ImageButton = () => {
  ...
  const handleImageUrlSubmit = () => {
    if (imageUrl) {
      ...
      toast.error("Enter URL properly!");
    }
  };
...
const ToolBarButton = ({
  ...
  return (
    <button
      onClick={onClick}
      className={cn(
        "text-sm h-7 px-2 flex items-center justify-center rounded-sm hover:bg-neutral-200/80",
        isActive && "bg-neutral-200/80"
      )}
    >
